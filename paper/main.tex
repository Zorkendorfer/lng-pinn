...
generator. We further show, in a controlled soft-vs-hard surrogate comparison,
that aggregate prediction accuracy is not sufficient for decision fidelity:
composition-dependent residual structure can create a spurious downstream
saving signal. The diagnostic motivates the hard-constrained architecture used
for the headline dispatch results.
\end{abstract}
...
\emph{decision} hazard. A soft-physics surrogate can leave a residual,
composition-dependent bias that is negligible in aggregate fidelity yet becomes a
systematic differential cost between two compared dispatch strategies, because the
strategies query the input space asymmetrically---one at a time-varying
composition, the other at a fixed one. A downstream optimiser can then read that
differential as a real signal. The controlled soft-vs-hard pipeline in this paper
is designed to test for exactly that hazard before the surrogate is trusted in a
decision loop; the hard architecture ties the cost-determining channel to exact
thermodynamics by construction.

This paper makes four contributions. (i)~We identify a
...
soft-vs-hard comparison and provide a cheap pre-deployment diagnostic that flags
the hazard before an optimiser is trusted. This is distinct from the prediction-accuracy arguments
...
reference value \SI{80}{\EUR\per\tCOtwo}, and is regime-conditional---absent in the 2022 crisis year. We
...
$p^{\text{co2}}=0$ to the reference range and then plateauing, so the
...
implemented and unit-tested in the released code, but the full CSTR grid is not
yet available in the manuscript artifacts. We therefore report only the
completed linear-ramp sweep and treat the headline as conditional on the
default tank-mixing dynamics.
...
under the default linear kernel. Under the default linear tank-mixing model,
the five-year mean saving is $+2.50\%$. Because this transition model generates
the information gap between the aware and lagged strategies, the result should
be interpreted as conditional on the assumed mixing dynamics. The upper end of
the sweep marks the edge of physical
...
\begin{table}[h]
  ...
  \caption{Five-year-mean composition-aware saving versus the lagged baseline
    ...
    $\tau_{\text{mix}}$ under the completed linear-ramp kernel ($n=50$
    seed-year cells per entry, $95\%$ Student's $t$ interval). The
    first-order CSTR robustness grid is implemented in code but is not reported
    here, so the headline should be read as conditional on the default mixing
    model.}
  \label{tab:mixing-sensitivity}
  \begin{tabular}{lc}
    \toprule
    $\tau_{\text{mix}}$ (d) & Linear-ramp saving \% [95\% CI] \\
    \midrule
    1  & $+1.62$ $[+1.08,+2.16]$ \\
    2  & $+1.71$ $[+1.16,+2.26]$ \\
    3  & $+2.12$ $[+1.56,+2.68]$ \\
    5  & $\mathbf{+2.51}$ $[+1.90,+3.12]$ \\
    7  & $+2.61$ $[+1.99,+3.22]$ \\
    10 & $+2.45$ $[+1.85,+3.04]$ \\
    \bottomrule
  \end{tabular}
...
\subsection{The decision hazard, controlled: soft constraints can create the signal}
...
$p_{\text{co}_2}\in\{0, \SI{80}{\EUR\per\tCOtwo}\}$. The final numerical table is
not included in the present artifact set, so we do not report a fabricated-saving
magnitude here. The controlled comparison is instead used as a decision-fidelity
diagnostic: a non-zero aware-versus-lagged saving under the soft surrogate at
zero carbon, paired with a near-zero hard-surrogate saving, would identify
composition-dependent surrogate residual structure rather than real process
economics.
...
The hazard generalises to any pipeline that uses a learned surrogate to compare
...
while the first-order CSTR grid remains to be reported. \textbf{Oracle foresight:} the aware
...
The transferable result of this study is a caution: a surrogate accurate enough
to pass every fidelity check can still fabricate a decision signal when it is used
to compare options that probe different regions of its input space, and the remedy
is architectural rather than a matter of tuning loss weights. We frame this in a
...
under exact physics---a hazard for any surrogate-in-the-loop optimisation that
compares decisions across different regions of the input space. The practical message for terminal operators is
...
markets. The diagnostic code also tests whether soft-physics surrogates can
create composition signals, and the sign of the headline saving is robust to completed
linear tank-mixing sweeps over $\tau_{\text{mix}}=\SIrange{1}{10}{\day}$ while
the first-order CSTR grid remains the key remaining robustness check. The
