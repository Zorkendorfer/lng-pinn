...
\textbf{$+2.58\%$ ($95\%$~CI $[+2.08, +3.07]$, $p=9.6\times10^{-7}$,
...
relative cost error below $10^{-4}$ on the dispatch trajectory; the explicit
CoolProp-versus-surrogate speed benchmark is reported with the released code.
\end{abstract}
...
is \textbf{$+2.58\%$} versus a realistic lagged-composition baseline
($95\%$~CI $[+2.08, +3.07]$, $p=9.6\times10^{-7}$, ten cargo-schedule seeds), and
...
combination. This validates the chain from Reality to CoolProp to surrogate;
direct validation against plant operating data remains future work. The
near-exact on-trajectory agreement is what licenses using the surrogate to
compare strategies that differ by only a few percent in cost---and, as
\cref{sec:methodological-warning} argues, is exactly what a soft-physics
...
$\mathbf{+2.58\%}$ over the realistic lagged-composition baseline when each
seed's five-year costs are pooled first
($95\%$~CI $[+2.08, +3.07]$, $t=11.7$, $p=9.6\times10^{-7}$, $n=10$ seeds).
...
\begin{table}[h]
  ...
  \caption{Seed sensitivity at \SI{80}{\EUR\per\tCOtwo}: per-year and
    ...
    cargo-schedule seeds. The five-year aggregate first pools costs within
    each seed, then tests the resulting 10 seed-level savings; the years are
    price regimes, not independent draws. SE is the standard error of the
  ...
  \begin{tabular}{lrrrrrr}
    ...
    \midrule
    \textbf{5-year pooled} & \textbf{10} & $\mathbf{+2.58}$ & $\mathbf{0.22}$ & $\mathbf{[+2.08, +3.07]}$ & $\mathbf{11.70}$ & $\mathbf{9.6 \times 10^{-7}}$ \\
    \bottomrule
  \end{tabular} \\[2pt]
...
plant operating data. \textbf{Inherited thermodynamic error:} the surrogate matches CoolProp to $<10^{-4}$ median
...
composition-aware dispatch saves $+2.58\%$ ($95\%$~CI $[+2.08, +3.07]$,
$p=9.6\times10^{-7}$, $n=10$ seeds) over a realistic lagged-composition baseline at the reference EU
...
the CSTR-kernel run remains the key remaining robustness check. Extending the
