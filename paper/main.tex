...
We present a physics-by-construction neural surrogate of FSRU regasification
in which the energy balance and pump-work relations hold \emph{exactly}, and
couple it to day-ahead electricity prices and an EU-ETS-style carbon cost in a
...
\textbf{$+2.50\%$ ($95\%$~CI $[+2.01, +2.98]$, $p=3.5\times10^{-6}$,
$n=10$ seeds)} over a realistic lagged-composition baseline at the reference
EU ETS price of \SI{80}{\EUR\per\tCOtwo}. Without carbon pricing the saving
collapses to zero. We further show that earlier soft-physics surrogates
produced spurious $4$--$7\%$ composition savings that vanish under hard
physics, suggesting a methodological pitfall relevant beyond LNG
regasification. The surrogate matches the CoolProp ground truth to a median
absolute relative cost error of $1.5\times10^{-7}$ on the dispatch trajectory
and runs approximately $1.1\times 10^4$ times faster than serial CoolProp HEOS
on a repeated 10,000-point benchmark.
\end{abstract}
...
CoolProp \citep{bell2014pure,huber2022refprop} is too slow for repeated use inside the
optimisation loop, which motivates a learned surrogate. Hybrid first-principles
plus neural process models have a long history \citep{psichogios1992hybrid},
and modern scientific machine learning frames this as the integration of
mechanistic constraints with data-driven approximation
\citep{karpatne2017theory_guided,karniadakis2021piml,willard2022scientific_ml}.
The approach differs
from classical PINNs in that physical constraints are embedded in the
architecture rather than enforced through PDE-residual terms; it is a
physics-by-construction surrogate within the broader SciML framework. This
distinction matters operationally: the standard soft-physics recipe---data-fit
loss plus soft penalty terms for the governing balances---turns out to be
actively dangerous in this setting. We show that a soft-physics surrogate
leaves residual, composition-dependent biases that a downstream optimiser
misreads as a real signal, fabricating apparent composition savings of
$4$--$7\%$ that vanish entirely when the physics is made exact. This is a
general hazard for any pipeline that uses a learned surrogate to compare
decisions querying different regions of the input space, and we believe it is
under-appreciated.

This paper makes four contributions. (i) We introduce a
\emph{physics-by-construction} surrogate architecture for FSRU regasification in
...
carbon pricing: at the reference EU ETS level of \SI{80}{\EUR\per\tCOtwo} it
is \textbf{$+2.50\%$} versus a realistic lagged-composition baseline
($95\%$~CI $[+2.01, +2.98]$, $p=3.5\times10^{-6}$, ten cargo-schedule seeds), and
...
methodological warning and contribution in its own right: soft constraints can
fabricate optimisation signals at exactly the scale being claimed. (iv) We provide a reproducible ten-seed sensitivity
...
absolute euros. Which carbon-pricing regime makes this a cost the dispatching
...
those hard constraints operational inside a downstream dispatch optimiser. We show in
...
surrogate to compare strategies that differ by only a few percent in cost---and,
as \cref{sec:methodological-warning} argues, is exactly what a soft-physics
surrogate fails to deliver.
...
\cref{tab:seed-significance}. At the reference EU ETS price of
...
\begin{figure}[t]
  ...
  \caption{Composition-aware dispatch saving versus carbon price,
    ...
    approximately $+1\%$ because emissions pin the optimal
...
\begin{figure}[htbp]
  ...
  \caption{Cross-year relationship between day-ahead price volatility
    ...
    The cross-year association is weak in correlation terms, so the figure
    should be read as regime structure rather than as a fitted mechanism:
    2022 is the crisis-year null, while the normal-price years recover the
    carbon-pricing lever.}
...
preserve the mole-fraction simplex exactly. For each
$(\tau_{\text{mix}},\text{kernel})$ cell we rerun the full ten-seed backtest at
$p^{\text{co2}}=\SI{80}{\EUR\per\tCOtwo}$ and report the five-year-mean saving
with its $95\%$ confidence interval (\cref{tab:mixing-sensitivity}).

The completed linear-ramp sweep spans $+1.62\%$ to $+2.61\%$ across
$\tau_{\text{mix}}=\SIrange{1}{10}{\day}$, with all confidence intervals
strictly above zero. This supports the sign of the composition-aware saving
under the default linear kernel, but the physically preferable CSTR kernel is
still being computed and should be promoted to the primary robustness claim
once complete. The upper end of the sweep marks the edge of physical
plausibility: as $\tau_{\text{mix}}$ approaches the
...
\begin{table}[h]
  ...
  \caption{Five-year-mean composition-aware saving versus the lagged baseline
    ...
    $\tau_{\text{mix}}$ and kernel ($n=50$ seed-year cells per completed entry,
    $95\%$ Student's $t$ interval). Linear-ramp values are from the completed
    cache in \texttt{data/processed/mixing\_sensitivity\_cells.parquet}. The
    CSTR sweep was still running at manuscript-edit time and is intentionally
    marked pending rather than interpolated.}
  \label{tab:mixing-sensitivity}
  \begin{tabular}{lcc}
    \toprule
    $\tau_{\text{mix}}$ (d) & Linear ramp & First-order (CSTR) \\
                            & saving \% [95\% CI] & saving \% [95\% CI] \\
    \midrule
    1  & $+1.62$ $[+1.08,+2.16]$            & pending \\
    2  & $+1.71$ $[+1.16,+2.26]$            & pending \\
    3  & $+2.12$ $[+1.56,+2.68]$            & pending \\
    5  & $\mathbf{+2.51}$ $[+1.90,+3.12]$   & pending \\
    7  & $+2.61$ $[+1.99,+3.22]$            & pending \\
    10 & $+2.45$ $[+1.85,+3.04]$            & pending \\
    \bottomrule
  \end{tabular}
\end{table}
...
delivered gas is priced---the EU ETS2 covering buildings and road transport
from 2027, or a prospective import-carbon charge analogous to the CBAM extended
...
reference value is chosen to coincide with the prevailing ETS allowance price
for concreteness, but the substantive result is the parametric sweep of
...
(\cref{fig:6_main}A); the temporal-arbitrage saving collapses from
approximately $20\%$ to approximately $1\%$ once any carbon price pins the
schedule (\cref{fig:6_main}B); and the per-window saving is concentrated in
price--composition regimes rather than in methane variability alone
...
\subsection{Methodological warning: soft-physics surrogates fabricate composition savings}
...
design. An earlier version of our pipeline used the conventional soft-physics recipe
...
leave residual biases at the approximately $1$--$5\%$ level---small in absolute fidelity
...
tuning the loss weights. A practical diagnostic is to compute the surrogate's
cost difference between a time-varying composition trajectory and its
window-mean composition, then compare that difference with the same calculation
from the ground-truth simulator. If the surrogate-only difference exceeds the
simulator difference by more than roughly $1$--$2\%$ of the simulator cost, the
architecture is likely fabricating a composition signal and should not be used
inside an optimiser without hard-constraint or on-trajectory validation.
...
completed linear-kernel part of this directly in \cref{sec:mixing-robustness},
with the CSTR kernel still pending. \textbf{Oracle foresight:} the aware
...
we quantify cargo-schedule variance with 10 seeds but do not propagate
property-model, weather, price-forecast, or demand uncertainty.
...
carbon pricing. Coupling a physics-by-construction neural surrogate to a
...
$p=3.5\times10^{-6}$, $n=10$ seeds) over a realistic lagged-composition baseline at the reference EU
...
the way we showed that the conventional soft-physics recipe fabricates a
...
markets. We also show that soft-physics surrogates can fabricate composition
signals, and that the sign of the headline saving is robust to completed
linear tank-mixing sweeps over $\tau_{\text{mix}}=\SIrange{1}{10}{\day}$ while
the CSTR-kernel run remains the key remaining robustness check. The
...
% \url{https://github.com/Zorkendorfer/lng-pinn}. Commit hash for the
% headline numbers reported here: \texttt{b3c2fd4}. The repository includes:
...
generated from commit \texttt{b3c2fd4} with Python~3.11, dependencies pinned by
\texttt{pyproject.toml} and \texttt{uv.lock}, and the trained checkpoint
\texttt{results/models/pinn\_v1.pt}. The independent cargo-schedule seeds are
\texttt{42, 0, 1, 7, 13, 19, 23, 31, 37, 53}. The ENTSO-E token used to fetch
day-ahead prices is read from \texttt{.env}; processed price/weather/cargo
tables and generated results are written under \texttt{data/processed/} and
\texttt{results/tables/}.

\textbf{Reproducing the headline number.} From a clean clone with the ENTSO-E
token configured, the following commands rebuild the data, train the surrogate,
rerun the 10-seed sensitivity, generate the carbon sweep, and redraw the
figures:
\begin{verbatim}
uv run python scripts/01_pull_entsoe.py --start 2021-01-01 \
    --end 2026-01-01 --zone LT
...
\end{verbatim}
and writes \texttt{results/tables/mixing\_sensitivity.csv} when complete. The
