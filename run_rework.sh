...
#   Phase 3 (soft + 4 bktests):~3 hours
#   Phase 4 (fabrication):    ~30 min
#   Phase 5 (carbon ensemble):~3–4 hours
#   Phase 6 (manifest+figs):  ~30 min
#   ----------------------------------
#   Full:                     ~10–12 hours
#
...
phase_cstr() {
  ...
  uv run python scripts/14_mixing_table.py
}

# ---------------------------------------------------------------------------
# PHASE 3 — item 2 (soft-vs-hard controlled comparison)
...
# ---------------------------------------------------------------------------
# PHASE 4 — item 3 (fabrication diagnostic, executable)
phase_fabrication() {
  for ARCH in hard soft; do
    MODEL_FLAG="results/models/pinn_v1.pt"
    [ "$ARCH" = "soft" ] && MODEL_FLAG="results/models/pinn_soft.pt"
...
Phases (independent, runnable in any order, idempotent via caches):
  ...
  soft            item 2 — train soft + 4 tagged backtests (~3 h)
  fabrication     item 3 — diagnostic per surrogate    (~30 min)
  ensemble        item 4 — 10 seeds × full carbon axis (~3–4 h)
  manifest        item 10 — write manifest + refresh figs (~30 min)
...
Typical workflow:
  ...
  ./run_rework.sh manifest       # 30 min — figures + manifest
EOF
    ;;
esac
